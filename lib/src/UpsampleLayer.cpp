...
namespace nvinfer1 {
UpsampleLayerPlugin::UpsampleLayerPlugin(const float scale,
                                         const int cudaThread /*= 512*/)
...
UpsampleLayerPlugin::UpsampleLayerPlugin(const void* data, size_t length) {
  ...
  // std::cout << "read:" << a << " " << mOutputWidth<< " "
  // <<mOutputHeight<<std::endl;
  assert(d == a + length);
}
...
void UpsampleLayerPlugin::serialize(void* buffer) {
  ...
  // std::cout << "write:" << a << " " << mOutputHeight<< " "
  // <<mOutputWidth<<std::endl;
...
Dims UpsampleLayerPlugin::getOutputDimensions(int index, const Dims* inputs,
                                              int nbInputDims) {
  // std::cout <<"Input:" << inputs[0].d[0] << " "<<inputs[0].d[1]<< "
  // "<<inputs[0].d[2]<<std::endl;
  // std::cout <<"nbInputDims : "<<nbInputDims<< " input:" << inputs[0].nbDims
  // << std::endl;

  mCHW = inputs[0];
  mOutputHeight = inputs[0].d[1] * mScale;
  mOutputWidth = inputs[0].d[2] * mScale;
  // std::cout << "ouputDims:" << mCHW.d[0] << " " << mOutputHeight << " " <<
  // mOutputWidth << std::endl;
